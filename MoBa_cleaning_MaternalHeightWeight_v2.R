...
q1=read.csv("~/Desktop/MoBa_v6/Q1_all.csv",h=F,sep=",",stringsAsFactors=F)
colnames(q1)=c("PREG_ID","AA85","AA86","AA87","AA88","AA89","AA93")
head(q1); dim(q1)
...
mpflags=data.frame(AA85=c(q1m$PREG_ID %in% remove85),AA86=c(q1m$PREG_ID %in% remove86),AA87=c(q1m$PREG_ID %in% remove87))
...
## attach per-mother averages and replace NAs with those
...
imp$AA85[is.na(imp$AA85)]=imp$a85[is.na(imp$AA85)]+d85*(-1)^imp$mp[is.na(imp$AA85)]
imp$AA86[is.na(imp$AA86)]=imp$a86[is.na(imp$AA86)]+d86*(-1)^imp$mp[is.na(imp$AA86)]
imp$AA87[is.na(imp$AA87)]=imp$a87[is.na(imp$AA87)]

## successful imputes:
sum(imp$AA85 != q1$AA85,na.rm=T)
sum(imp$AA86 != q1$AA86,na.rm=T)
sum(imp$AA87 != q1$AA87,na.rm=T)
